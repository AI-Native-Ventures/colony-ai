...
pub enum IdentityMode {
    /// A diagnostic/health launch that must not initialize an identity.
    HealthOnly,
    /// An explicitly authorized profile launch that may initialize identity.
    ExplicitIdentity,
}

/// Provenance for the reset flag carried into the metadata projection.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ResetProvenance {
    /// No reset was requested for this launch.
    NotAttemptedFresh,
    /// A reset completed with a verified outcome.
    VerifiedSuccess,
    /// A reset was attempted and failed with a durable failure state.
    VerifiedFailure,
    /// The caller cannot prove the reset outcome yet.
    Unknown,
}

/// Manifest namespace values supplied by a trusted main/packaging boundary.
///
/// This type intentionally contains no renderer or environment input. The
/// constructor is useful for synthetic fixtures; production callers should
/// build it only from the checked-in flavor manifest.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct TrustedProfileManifest {
    profile_id: String,
    flavor: String,
    platform: String,
...
impl TrustedProfileManifest {
    /// Construct a manifest fixture or a value decoded from a trusted static
    /// manifest. Runtime untrusted payloads must not call this directly.
    pub fn new(
...
/// Errors raised before a profile can reach the keyring boundary.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl std::fmt::Display for DescriptorError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::InvalidMode => formatter.write_str("identity mode is not explicit"),
            Self::InvalidProfile => formatter.write_str("invalid profile namespace"),
            Self::InvalidFlavor => formatter.write_str("invalid build flavor"),
            Self::InvalidPlatform => formatter.write_str("invalid host platform"),
            Self::InvalidService => formatter.write_str("invalid keyring service namespace"),
            Self::InvalidUserDataRoot => formatter.write_str("invalid user-data root"),
            Self::SymlinkEscape => {
                formatter.write_str("profile path symlink escapes user-data root")
            }
            Self::Filesystem(error) => formatter.write_str(error),
        }
    }
}
...
/// A validated, main-owned identity launch descriptor.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct IdentityLaunchDescriptor {
    profile_id: String,
    flavor: String,
    platform: String,
...
impl IdentityLaunchDescriptor {
    /// Validate manifest-derived namespace values and derive all profile paths.
    ///
    /// The root is canonicalized and existing identity/marker symlinks are
    /// rejected. No identity initialization or keyring access occurs here.
    pub fn from_trusted_manifest(
        ...
        if manifest.profile_id.is_empty()
            || manifest.profile_id.len() > 96
            || !safe_namespace_component(&manifest.profile_id)
        {
            return Err(DescriptorError::InvalidProfile);
        }
        if !matches!(manifest.flavor.as_str(), "normal" | "instrumented" | "test")
            || !safe_namespace_component(&manifest.flavor)
        {
            return Err(DescriptorError::InvalidFlavor);
        }
        if identity_mode != IdentityMode::ExplicitIdentity {
            return Err(DescriptorError::InvalidMode);
        }
        if shared_identity {
            return Err(DescriptorError::InvalidMode);
        }
        if !matches!(manifest.platform.as_str(), "macos" | "windows" | "linux") {
            return Err(DescriptorError::InvalidPlatform);
        }
        if manifest.keyring_service.is_empty()
            || manifest.keyring_service.len() > 128
            || !safe_service_name(&manifest.keyring_service)
            || matches!(
                manifest.keyring_service.as_str(),
        ...
        Ok(Self {
            ...
            user_data_root: canonical_root,
            keyring_service: manifest.keyring_service.clone(),
            identity_mode,
            shared_identity,
            reset_provenance,
        })
    ...
    /// Return the trusted build flavor bound to this profile.
    pub fn flavor(&self) -> &str {
        &self.flavor
    }

    /// Return the target platform bound to this profile.
    pub fn platform(&self) -> &str {
        &self.platform
    }

    /// Return the canonical profile data root.
    pub fn user_data_root(&self) -> &Path {
        &self.user_data_root
    }

    /// Return the manifest-derived OS keyring service namespace.
    pub fn keyring_service(&self) -> &str {
        &self.keyring_service
    }

    /// Return the launch mode validated for this descriptor.
    pub fn identity_mode(&self) -> IdentityMode {
        self.identity_mode
    }

    /// Return whether this descriptor was marked as shared identity.
    pub fn shared_identity(&self) -> bool {
        self.shared_identity
    }

    /// Return reset provenance without treating an unknown outcome as success.
    pub fn reset_provenance(&self) -> ResetProvenance {
        self.reset_provenance
    }

    /// Return the profile-local 0600 file fallback path.
    pub fn identity_file_path(&self) -> PathBuf {
        self.user_data_root.join(IDENTITY_FILE_NAME)
    }

    /// Return the profile-local migration marker path.
    pub fn migration_marker_path(&self) -> PathBuf {
        self.user_data_root
            .join(colony_identity_kernel::migration_marker_name(
                &self.keyring_service,
                MIGRATION_MARKER_NAME,
            ))
    }

    /// Return the kernel scope carrying only this descriptor's paths/service.
    pub fn profile_scope(&self) -> colony_identity_kernel::ProfileScope {
        colony_identity_kernel::ProfileScope::new(
            self.user_data_root.clone(),
            self.identity_file_path(),
            self.keyring_service.clone(),
...
fn safe_service_name(value: &str) -> bool {
    safe_namespace_component(value) && !value.contains('/') && !value.contains('\\')
}

/// Fixed-operation errors for the headless identity store. Variants contain
/// no backend payload or secret material and are safe to cross a redacted
/// diagnostic boundary.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum HeadlessStoreError {
    ...
    Missing,
    /// The backend is not reachable for this operation.
    Unreachable,
    /// The backend is locked for this operation.
    Locked,
    /// A pre-existing blob failed strict decoding.
    CorruptCurrentBlob,
    /// Direct read-back returned a different identity.
    ReadbackMismatch,
    /// The backend rejected an identity write.
    BackendWrite,
    /// The backend rejected an identity delete.
    BackendDelete,
}

impl std::fmt::Display for HeadlessStoreError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.write_str(match self {
            ...
            Self::Locked => "identity_store_locked",
            Self::CorruptCurrentBlob => "identity_blob_corrupt",
            Self::ReadbackMismatch => "identity_readback_mismatch",
            Self::BackendWrite => "identity_store_write_failed",
            Self::BackendDelete => "identity_store_delete_failed",
        })
    }
}

impl std::error::Error for HeadlessStoreError {}

fn map_backend_error(error: &str, delete: bool) -> HeadlessStoreError {
    #[cfg(feature = "system-keyring")]
    if is_keyring_locked_error(error) {
        HeadlessStoreError::Locked
    } else if is_keyring_availability_error(error) {
        HeadlessStoreError::Unreachable
    } else if delete {
        HeadlessStoreError::BackendDelete
    } else {
        HeadlessStoreError::BackendWrite
    }
    #[cfg(not(feature = "system-keyring"))]
    {
        let _ = (error, delete);
...
fn decode_headless_blob(raw: &[u8]) -> Result<HashMap<String, String>, HeadlessStoreError> {
    let value = serde_json::from_slice::<serde_json::Value>(raw)
        .map_err(|_| HeadlessStoreError::CorruptCurrentBlob)?;
    let object = value
        .as_object()
        .ok_or(HeadlessStoreError::CorruptCurrentBlob)?;
    if let Some(version) = object.get("version") {
        let known_version = version.as_u64() == Some(1) || version.as_str() == Some("1");
        if !known_version {
            return Err(HeadlessStoreError::CorruptCurrentBlob);
        }
    }
    ...
    if let Some(secret) = map.get(colony_identity_kernel::IDENTITY_KEY_NAME) {
        Keys::parse(secret.trim()).map_err(|_| HeadlessStoreError::CorruptCurrentBlob)?;
    }
    Ok(map)
}

fn encode_headless_blob(map: &HashMap<String, String>) -> Result<Vec<u8>, HeadlessStoreError> {
    serde_json::to_vec(map).map_err(|_| HeadlessStoreError::BackendWrite)
}

fn compare_headless_identity(map: &HashMap<String, String>, expected: &str) -> HeadlessReadback {
    let expected_keys = match Keys::parse(expected.trim()) {
        Ok(keys) => keys,
        Err(_) => return HeadlessReadback::Mismatch,
    };
    let Some(actual) = map.get(colony_identity_kernel::IDENTITY_KEY_NAME) else {
        return HeadlessReadback::Missing;
    };
    let actual_keys = match Keys::parse(actual.trim()) {
        Ok(keys) => keys,
        Err(_) => return HeadlessReadback::CorruptCurrentBlob,
    };
    if actual_keys.public_key() == expected_keys.public_key()
        && actual_keys.secret_key().as_secret_bytes()
            == expected_keys.secret_key().as_secret_bytes()
    {
                ...
                .map_err(|error| map_backend_error(&error, false))?
            {
                ...
                Some(raw) => match decode_headless_blob(&raw) {
                    ...
                    Err(HeadlessStoreError::CorruptCurrentBlob) if value.is_none() => {
                        self.delete_headless_blob_locked()?;
                        return Ok(());
                    }
...
/// Strict identity-only facade over the shared platform blob backend.
pub struct HeadlessIdentityStore {
...
impl HeadlessIdentityStore {
    /// Bind one store/cache/lock namespace to a validated explicit profile.
    ...
    /// Return the validated launch descriptor.
    pub fn descriptor(&self) -> &IdentityLaunchDescriptor {
        &self.descriptor
    }

    /// Return the kernel scope bound to this profile.
    pub fn profile_scope(&self) -> colony_identity_kernel::ProfileScope {
    ...
    fn set_identity_cache(&self, value: Option<String>) {
        let mut guard = self
            .identity_cache
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    pub fn probe_identity(&self) -> HeadlessProbe {
        match self.load_identity_cached() {
            ...
            Err(HeadlessStoreError::Locked) => HeadlessProbe::Locked,
            Err(HeadlessStoreError::Unreachable) => HeadlessProbe::Unreachable,
            Err(_) => HeadlessProbe::Unreachable,
        }
    }

    /// Load only the fixed identity field through the profile cache.
    pub fn load_identity(&self) -> Result<Option<String>, HeadlessStoreError> {
        self.load_identity_cached()
    }

    /// Store one validated identity field using the profile lock and blob
    ...
    /// Resolve a fresh profile through the shared kernel with strict read-back
    /// semantics; this does not wire the health-only native host startup path.
    pub fn initialize(
        &self,
    ) -> Result<
...
impl colony_identity_kernel::IdentityKeyStore for HeadlessIdentityStore {
    fn probe(&self, name: &str) -> KeyringProbe {
        if name != colony_identity_kernel::IDENTITY_KEY_NAME {
            return KeyringProbe::Unreachable;
        }
        match self.probe_identity() {
            HeadlessProbe::Present => KeyringProbe::Present,
            HeadlessProbe::Missing => KeyringProbe::ReachableButEmpty,
            HeadlessProbe::CorruptCurrentBlob => KeyringProbe::CorruptCurrentBlob,
            HeadlessProbe::Unreachable | HeadlessProbe::Locked => KeyringProbe::Unreachable,
        }
    }
    ...
    fn load(&self, name: &str) -> Result<Option<String>, String> {
        ...
        self.load_identity().map_err(|error| error.to_string())
    }
    ...
    fn delete(&self, name: &str) -> Result<(), String> {
        ...
        self.delete_identity().map_err(|error| error.to_string())
    }
    ...
    fn verify_stored(&self, key: &str, expected: &str) -> Result<bool, String> {
        ...
        match self.verify_identity(expected) {
            HeadlessReadback::Exact => Ok(true),
            HeadlessReadback::Mismatch | HeadlessReadback::Missing => Ok(false),
            HeadlessReadback::CorruptCurrentBlob => {
                Err(HeadlessStoreError::CorruptCurrentBlob.to_string())
            }
            HeadlessReadback::Unreachable => Err(HeadlessStoreError::Unreachable.to_string()),
            HeadlessReadback::Locked => Err(HeadlessStoreError::Locked.to_string()),
        }
    }

    fn verify_stored_headless(&self, key: &str, expected: &str) -> HeadlessReadback {
        if key != colony_identity_kernel::IDENTITY_KEY_NAME {
            return HeadlessReadback::Mismatch;
        }
...
mod headless_tests {
    ...
    fn headless_decoder_types_malformed_and_unknown_blobs() {
        assert_eq!(
            decode_headless_blob(&[0xff, 0x00]),
            Err(HeadlessStoreError::CorruptCurrentBlob)
        );
        assert_eq!(
            decode_headless_blob(br#"{"identity":""#),
            Err(HeadlessStoreError::CorruptCurrentBlob)
        );
        assert_eq!(
            decode_headless_blob(br#"{"version":2,"identity":"x"}"#),
            Err(HeadlessStoreError::CorruptCurrentBlob)
        );
        assert_eq!(
            decode_headless_blob(br#"{"identity":"not-a-secret"}"#),
            Err(HeadlessStoreError::CorruptCurrentBlob)
        );
    }

    #[test]
    fn headless_decoder_accepts_legacy_map_and_known_version() {
        let keys = Keys::generate();
        let nsec = keys.secret_key().to_bech32().expect("synthetic nsec");
        let mut input = HashMap::new();
        input.insert(
            colony_identity_kernel::IDENTITY_KEY_NAME.to_string(),
            nsec.clone(),
        );
        let raw = encode_headless_blob(&input).expect("synthetic blob");
        let map = decode_headless_blob(&raw).expect("known blob");
        assert_eq!(
            map.get(colony_identity_kernel::IDENTITY_KEY_NAME),
            Some(&nsec)
        );

        let different = Keys::generate();
        let different_nsec = different.secret_key().to_bech32().expect("synthetic nsec");
        ...
        assert_eq!(
            compare_headless_identity(&mismatched, &nsec),
            HeadlessReadback::Mismatch
        );
    }

    #[test]
    fn descriptor_rejects_health_mode_and_legacy_service() {
        let root = tempfile::tempdir().expect("isolated root");
        let health = TrustedProfileManifest::new(
            "fresh-profile",
            "test",
            "linux",
        ...
        assert_eq!(
            IdentityLaunchDescriptor::from_trusted_manifest(
                &health,
                root.path(),
                IdentityMode::HealthOnly,
                false,
                ResetProvenance::NotAttemptedFresh,
            )
            .expect_err("health-only descriptor must not initialize identity"),
            DescriptorError::InvalidMode
        );

        let legacy = TrustedProfileManifest::new("fresh-profile", "test", "linux", "buzz-desktop");
        assert_eq!(
            IdentityLaunchDescriptor::from_trusted_manifest(
                &legacy,
                root.path(),
            ...
            .expect_err("headless path must not open the legacy service"),
            DescriptorError::InvalidService
        );
    }

    #[test]
    fn descriptor_derives_confined_profile_scope_without_key_access() {
        let root = tempfile::tempdir().expect("isolated root");
        ...
        let descriptor = IdentityLaunchDescriptor::from_trusted_manifest(
            ...
            IdentityMode::ExplicitIdentity,
            false,
            ResetProvenance::Unknown,
        )
        .expect("trusted descriptor");
        assert_eq!(descriptor.profile_id(), "fresh-profile");
        assert_eq!(
            descriptor.profile_scope().data_dir(),
            descriptor.user_data_root()
        );
        assert!(descriptor
            .identity_file_path()
