...
const REAL_HOST_OPTIONS = {
  skip: BINARY
    ? false
    : "COLONY_NATIVE_HOST_BIN is not set; hosted Rust interop only",
};
...
test(
  "real Rust host serves the health-safe request and preserves one process across repeated rebinds",
  REAL_HOST_OPTIONS,
  async () => {
...
test(
  "real Rust host fences a delayed old request before REBOUND and never replays it",
  REAL_HOST_OPTIONS,
  async () => {
...
test(
  "real Rust startup failures are bounded and redacted into stable protocol codes",
  REAL_HOST_OPTIONS,
  async () => {
...
test(
  "real Rust host death settles pending work once",
  REAL_HOST_OPTIONS,
  async () => {
...
function spawnRawHost() {
  ...
  child.stdout.on("end", () => {
    while (waiters.length > 0) {
      waiters
        .shift()
        .reject(streamError ?? new Error("real host closed before a frame"));
    }
  });
  child.readFrame = () => {
    if (bufferedFrames.length > 0)
      return Promise.resolve(bufferedFrames.shift());
...
test(
  "real Rust host accepts slow partial writes and fails closed at the frame limit",
  REAL_HOST_OPTIONS,
  async () => {
